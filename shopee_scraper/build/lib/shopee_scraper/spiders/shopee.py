...
class ShopeeSpider(scrapy.Spider):
                ...
                for sub_sub in sub["sub_sub"]:
                    ...
                    for pagenum in range(max_pagenum):
                        ...
                        if url_page in self.scraped_urls:
                            print('Already Scraped URL Page\t', url_page)
                            continue
    ...
    def parse(self, response):
        ...
        if url:
            ...
            if len(items) == 0:
                return
            print("{} items scraped".format(len(items)), "\t", url)
            for item in items:
...
if __name__ == '__main__':
    ...
    str_today = date.today().strftime("%Y%m%d")
    cmdline.execute("scrapy crawl shopee -o shopee_{}.json".format(str_today).split())